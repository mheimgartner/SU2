...
class Viscosity {
    ...
    public: 
        ...
        double wilkeViscosity(std::vector<double> massFractions, std::vector<double> &molarMasses, std::vector<double> &viscosityConstant, int numberOfComponents);
        double davidsonViscosity(std::vector<double> massFractions, std::vector<double> &molarMasses, std::vector<double> &viscosityConstant, int numberOfComponents);
}; 
...
double Viscosity::wilkeViscosity(std::vector<double> massFractions, std::vector<double> &molarMasses, std::vector<double> &viscosityConstant, int numberOfComponents){
    ...
    for(int i = 0; i < numberOfComponents; i++){
        for(int j = 0; j < numberOfComponents; j++){
            phi.push_back(pow((1 + pow(viscosityConstant[i] / viscosityConstant[j],0.5) * pow(molarMasses[j] / molarMasses[i],0.25)),2) / pow(8 * (1 + molarMasses[i] / molarMasses[j]),0.5)); 
            wilkeDenumerator += moleFractions[j] * phi[j];   
        }
        wilkeDenumeratorSum.push_back(wilkeDenumerator); 
        wilkeDenumerator = 0.0; 
        phi.clear();
        wilkeNumerator.push_back(moleFractions[i] * viscosityConstant[i]); 
        viscosityMixture += wilkeNumerator[i] / wilkeDenumeratorSum[i];  
    }
    return viscosityMixture; 
}

double Viscosity::davidsonViscosity(std::vector<double> massFractions, std::vector<double> &molarMasses, std::vector<double> &viscosityConstant, int numberOfComponents){
    double fluidity = 0.0;
    double E = 0.0; 
    double mixtureFractionDenumerator = 0.0;
    ...
    for(int i = 0; i < numberOfComponents; i++){
        for(int j = 0; j < numberOfComponents; j++){
                ...
                fluidity += ((mixtureFractions[i] * mixtureFractions[j]) / (sqrt(viscosityConstant[i]) * sqrt(viscosityConstant[j]))) * pow(E, A);  
        }
    }
